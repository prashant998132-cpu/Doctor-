...
export class ReminderManager {
  ...
  parseNaturalTime(text: string): number | null {
    ...
    const tomorrowMatch = text.match(/tomorrow\s+at\s+(\d+):(\d+)/i)
    if (tomorrowMatch) {
      const h = parseInt(tomorrowMatch[1])
      const mi = parseInt(tomorrowMatch[2])
    ...
    if (hoursMatch) {
      return now + parseInt(hoursMatch[1]) * 3600000
    }

    const minsMatch = text.match(/in\s+(\d+)\s+minutes?/i)
    if (minsMatch) {
      return now + parseInt(minsMatch[1]) * 60000
    }
    ...
    const weekMatch = text.match(/next\s+week/i)
    if (weekMatch) {
      const date = new Date()
