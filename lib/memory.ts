...
function checkAndAwardBadges(r: Relationship): void {
  // Badge system - currently simplified
  // Can be extended later with more badges
  if (r.level >= 5) {
    r.badges = r.badges || []
    if (!r.badges.includes('master')) r.badges.push('master')
  }
  if (r.totalInteractions >= 100 && !r.badges?.includes('talker')) {
    r.badges = r.badges || []
    r.badges.push('talker')
  }
}
